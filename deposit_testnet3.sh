...
# id="$(${ironfish_cmd} config:get blockGraffiti | sed -e "s/^.//;s/.$//")"             # use for NPM

ironfish_cmd="/usr/bin/yarn --cwd ${HOME}/ironfish/ironfish-cli/ ironfish"              # use for SRC
id="$(${ironfish_cmd} config:get blockGraffiti | sed -n '3p' | sed -e "s/^.//;s/.$//")" # use for SRC
...
${ironfish_cmd} wallet:burn -a=90 -f=${wallet} -o=${fee} --assetId=${asset_id} --confirm
